...
public class DiamondGenerator implements Generator {
    ...
    public DiamondGenerator(Location loc) {
        level = 1;
        EFFICIENT = 90 * 20;
        this.loc = loc;
        ArrayList<Entity> entities = loc.getWorld().getNearbyEntities(loc, 1, 1, 1).stream()
                .filter(e -> e.getType().equals(EntityType.ARMOR_STAND) && e.getScoreboardTags().contains("diamondGenerator"))
        ...
        Armor.setCustomName("§e下一顆鑽石將在 §a- §e秒後出現");
        Armor.setSmall(true);
        Armor.setCustomNameVisible(true);
        Armor.setHelmet(new ItemStack(Material.DIAMOND_BLOCK));
        Armor.addScoreboardTag("diamondGenerator");

        ArrayList<Entity> entities2 = loc.getWorld().getNearbyEntities(loc, 1, 2, 1).stream()
                .filter(e -> e.getType().equals(EntityType.ARMOR_STAND) && e.getScoreboardTags().contains("diamondGenerator_Title"))
                .collect(Collectors.toCollection(ArrayList::new));
        ...
        Title.setCustomName("§6鑽石生成器 - 等級" + Round.Rome(level));
        Title.setSmall(true);
        Title.setCustomNameVisible(true);
        Title.addScoreboardTag("diamondGenerator_Title");
    }
    ...
    public void generate() {
        TICKPAST = 0;
        loc.getWorld().dropItemNaturally(loc,new ItemStack(ItemBank.ENCHANTED_DIAMOND)).addScoreboardTag("diamond");
    }
    ...
    public void spin() {
        ...
        Armor.setCustomName("§e下一顆鑽石將在 §a"+((EFFICIENT/20) - (TICKPAST/20))+" §e秒後出現");
